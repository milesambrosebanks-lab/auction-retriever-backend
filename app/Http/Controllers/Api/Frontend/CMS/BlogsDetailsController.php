...
class BlogsDetailsController extends Controller
{
    public function index()
    {
        ...
        $cmsItems           = CMS::where('page', PageEnum::BLOGSDETAILS)->where('status', 'active')->get();
        $cmsCommon          = CMS::where('page', PageEnum::COMMON)->where('status', 'active')->get();

        $data['blogsdetails_banner']                  = $cmsItems->where('section', SectionEnum::BLOGSDETAILS_BANNER)->first();
        $data['blogsdetails_related_post']            = $cmsItems->where('section', SectionEnum::BLOGSDETAILS_RELATED_POST)->first();
