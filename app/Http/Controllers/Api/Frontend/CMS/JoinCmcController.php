...
class JoinCmcController extends Controller
{
    public function index()
    {
        $data = [];

        $cmsItems           = CMS::where('page', PageEnum::JOIN_CMC)->where('status', 'active')->get();
        $cmsCommon          = CMS::where('page', PageEnum::COMMON)->where('status', 'active')->get();

        $data['join_cmc_banner']                  = $cmsItems->where('section', SectionEnum::JOIN_CMC_BANNER)->first();
        $data['join_cmc_form']                    = $cmsItems->where('section', SectionEnum::JOIN_CMC_FORM)->first();
