...
class RepresentativeController extends Controller
{
    public function index()
    {
        ...
        $cmsItems           = CMS::where('page', PageEnum::REPRESENTATIVE)->where('status', 'active')->get();
        $cmsCommon          = CMS::where('page', PageEnum::COMMON)->where('status', 'active')->get();

        $data['representative_banner']          = $cmsItems->where('section', SectionEnum::REPRESEMENTATIVE_BANNER)->first();
        $data['representative_form']            = $cmsItems->where('section', SectionEnum::REPRESEMENTATIVE_FORM)->first();
        ...
        return Helper::jsonResponse(true, 'Representative Page', 200, $data);
