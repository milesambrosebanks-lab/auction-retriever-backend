...
class ShopDetailsController extends Controller
{
    public function index()
    {
        ...
        $cmsItems           = CMS::where('page', PageEnum::SHOPDETAILS)->where('status', 'active')->get();
        $cmsCommon          = CMS::where('page', PageEnum::COMMON)->where('status', 'active')->get();

        $data['shopdetails_banner']                  = $cmsItems->where('section', SectionEnum::SHOPDETAILS_BANNER)->first();
        $data['shopdetails_related_post']            = $cmsItems->where('section', SectionEnum::SHOPDETAILS_RELATED_POST)->first();
