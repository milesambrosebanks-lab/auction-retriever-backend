...
class EventsDetailsController extends Controller
{
    public function index()
    {
        $data = [];

        $cmsItems           = CMS::where('page', PageEnum::EVENTSDETAILS)->where('status', 'active')->get();
        $cmsCommon          = CMS::where('page', PageEnum::COMMON)->where('status', 'active')->get();

        $data['eventsdetails_banner']                  = $cmsItems->where('section', SectionEnum::EVENTSDETAILS_BANNER)->first();
        $data['eventsdetails_registration']            = $cmsItems->where('section', SectionEnum::EVENTSDETAILS_REGISTRATION)->first();
        $data['eventsdetails_related_post']            = $cmsItems->where('section', SectionEnum::EVENTSDETAILS_RELATED_POST)->first();
