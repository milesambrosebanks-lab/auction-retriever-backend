...
class BlogsController extends Controller
{
    public function index()
    {
        ...
        $cmsItems           = CMS::where('page', PageEnum::BLOGS)->where('status', 'active')->get();
        $cmsCommon          = CMS::where('page', PageEnum::COMMON)->where('status', 'active')->get();

        $data['blogs_banner']                  = $cmsItems->where('section', SectionEnum::BLOGS_BANNER)->first();
        $data['blogs_list']                    = $cmsItems->where('section', SectionEnum::BLOGS_LIST)->first();
