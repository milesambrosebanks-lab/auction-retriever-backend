...
class NewsDetailsController extends Controller
{
    public function index()
    {
        ...
        $cmsItems           = CMS::where('page', PageEnum::NEWSDETAILS)->where('status', 'active')->get();
        $cmsCommon          = CMS::where('page', PageEnum::COMMON)->where('status', 'active')->get();

        $data['newsdetails_banner']                  = $cmsItems->where('section', SectionEnum::NEWSDETAILS_BANNER)->first();
        $data['newsdetails_related_post']            = $cmsItems->where('section', SectionEnum::NEWSDETAILS_RELATED_POST)->first();
