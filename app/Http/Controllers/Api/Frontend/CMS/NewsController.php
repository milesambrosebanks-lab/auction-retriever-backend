...
class NewsController extends Controller
{
    public function index()
    {
        ...
        $cmsItems           = CMS::where('page', PageEnum::NEWS)->where('status', 'active')->get();
        $cmsCommon          = CMS::where('page', PageEnum::COMMON)->where('status', 'active')->get();

        $data['news_banner']                  = $cmsItems->where('section', SectionEnum::NEWS_BANNER)->first();
        $data['news_list']                    = $cmsItems->where('section', SectionEnum::NEWS_LIST)->first();
