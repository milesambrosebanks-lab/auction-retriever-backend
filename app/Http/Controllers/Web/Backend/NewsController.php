...
class NewsController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = News::with(['user'])->orderBy('id', 'desc')->get();
            return DataTables::of($data)
                ...
                ->rawColumns(['author', 'title', 'thumbnail', 'status', 'action'])
                ->make();
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $news->slug = Helper::makeSlug(News::class, $data['title']);
            ...
            $news->type = $data['type'];
            $news->created_at = $data['created_at'] ?? now();
            $news->save();
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $news->title = $data['title'];
            $news->thumbnail = $data['thumbnail'] ?? $news->thumbnail;
            $news->content = $data['content'] ?? null;
            $news->link = $data['link'] ?? null;
            $news->type = $data['type'];
            $news->created_at = $data['created_at'] ?? now();
