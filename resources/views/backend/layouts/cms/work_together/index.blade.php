        ...
        <div class="main-container container-fluid">
            ...
            <div class="page-header">
                ...
                <div class="ms-auto pageheader-btn">
                    <button onclick="window.location.href=`{{ route($url . '.display', $data->id??1) }}`" class="btn me-2 {{ isset($data->is_display) && $data->is_display == 0 ? 'btn-danger' : 'btn-primary' }}">Display</button>
                </div>
            </div>
            <!-- PAGE-HEADER END -->
                        ...
                        <div class="card-body">
                            <form class="form form-horizontal" method="POST" action="{{ route($url.'.content') }}" enctype="multipart/form-data">
                                ...
                                <div class="row {{ in_array('sub_title', $component) ? '' : 'd-none' }}">
                                    <div class="col-md-12">
                                        <x-form.text name="sub_title" label="Designation" placeholder="Enter designation" :value="$data->sub_title ?? ''" />
                                    </div>
                                </div>

                                <div class="row {{ in_array('description', $component) ? '' : 'd-none' }}">
                                    <div class="col-md-12">
                                        <x-form.textarea name="description" label="Description 1" placeholder="Enter description 1" :value="$data->description ?? ''" />
                                    </div>
                                </div>

                                <div class="row {{ in_array('sub_description', $component) ? '' : 'd-none' }}">
                                    <div class="col-md-12">
                                        <x-form.textarea name="sub_description" label="Description 2" placeholder="Enter description 2" :value="$data->sub_description ?? ''" />
                                    </div>
                                </div>
                                <div class="row {{ in_array('others', $component) ? '' : 'd-none' }}">
                                    <div class="col-md-12">
                                        <x-form.textarea name="others" label="Description 3" placeholder="Enter description 3" :value="$data->others ?? ''" />
                                    </div>
                                </div>
