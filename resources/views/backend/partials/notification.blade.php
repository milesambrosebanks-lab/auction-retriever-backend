...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var user = `{{ auth('web')->check() ? auth('web')->user()->id : null }}`;

        if (user) {

            Echo.private(`notify.{{ auth('web')->user()->id }}`).listen('RegistrationNotificationEvent', (e) => {
                toastr.success(e.data.name + ' ' + e.data.body);
                notification();
            });

            Echo.private(`test-notify.{{ auth('web')->user()->id }}`).listen('TestNotificationEvent', (e) => {
